...
<section class="bg-gradient-to-r from-purple-500 to-pink-500 h-40 flex items-center justify-center shadow-lg">
    <h1 class="text-6xl md:text-7xl font-extrabold text-white text-center">
        EDIT <span class="text-pink-800 drop-shadow-lg">NEW</span> WORK EXPERIENCE !!!
    </h1>
</section>

<div class="container mx-auto px-4 py-12">
    <form action="{{ route('admin.experience.update', $experience) }}" enctype="multipart/form-data" method="POST"
        class="space-y-12">
        @csrf
        @method('PUT');

        <section class="bg-gray-800 p-8 md:p-12 rounded-2xl shadow-2xl space-y-8">
            <h2 class="text-3xl font-bold text-white mb-6 border-b-2 border-purple-500 pb-2">Experience Details of {{$experience->name}}</h2>

            <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                <div class="flex flex-col">
                    <label for="name" class="text-gray-300 font-semibold mb-2">Company Name</label>
                    <input type="text" id="name" name="name" value="{{$experience->name}}"
                ...
                <div class="flex flex-col">
                    ...
                    <input type="text" id="tagline" name="tagline" value="{{$experience->tagline}}"
                        class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300 placeholder-gray-400"
                ...
                <div class="flex flex-col">
                    <label for="status" class="text-gray-300 font-semibold mb-2">Status</label>
                    <select id="status" name="status"
                        class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300">
                        <option value="" disabled selected class="text-gray-400">{{$experience->status}}</option>
                ...
                <div class="flex flex-col">
                    <label for="datestart" class="text-gray-300 font-semibold mb-2">Start Date</label>
                    <input type="date" id="datestart" name="datestart" value="{{$experience->datestart}}"
                        class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300">
                    @error('datestart')
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
                ...
                <div class="flex flex-col">
                    <label for="dateend" class="text-gray-300 font-semibold mb-2">End Date</label>
                    <input type="date" id="dateend" name="dateend" value="{{$experience->dateend}}"
                        class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300">
                    @error('dateend')
                        <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                    @enderror
        ...
        <section class="bg-gray-800 p-8 md:p-12 rounded-2xl shadow-2xl space-y-8">
            ...
            <div class="flex flex-col">
                ...
                <textarea id="description" name="description" rows="6"
                    class="w-full bg-gray-700 text-white border border-gray-600 rounded-lg py-3 px-4 focus:outline-none focus:ring-2 focus:ring-purple-500 transition duration-300 placeholder-gray-400"
                    placeholder="Tell us about your Experiences...">{{$experience->description}}</textarea>
                @error('description')
                    <p class="text-red-500 text-sm mt-2">{{ $message }}</p>
                @enderror
            </div>
