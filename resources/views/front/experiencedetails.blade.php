...
<section class="bg-gradient-to-t from-itam to-dasar min-h-screen text-white font-inter">
    ...
    <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <nav class="mb-12">
            <a href="{{ route('experience') }}" class="inline-flex items-center gap-2 text-mera font-bold text-lg md:text-xl hover:text-white transition-colors duration-300">
        ...
        <header class="text-center flex flex-col items-center gap-4">
            <div class="p-2 bg-white/10 rounded-full shadow-lg">
                <img src="{{ Storage::url($experience->cover) }}" alt="{{ $experience->name }} Logo" class="w-28 h-28 sm:w-32 sm:h-32 md:w-40 md:h-40 object-contain rounded-full">
            </div>
            <h1 class="mt-4 font-bold text-3xl md:text-5xl">{{ $experience->name }}</h1>
            <p class="text-gray-300 text-base md:text-lg max-w-2xl">{{ $experience->tagline }}</p>
        </header>

        <section class="mt-16 md:mt-20">
            <h2 class="text-center text-2xl md:text-3xl font-bold mb-6">About This Experience</h2>
        ...
        <section class="mt-16 md:mt-20">
            ...
            @if($experience->screenshots->isNotEmpty())
                <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 md:gap-8">
                    {{-- The loop was fixed here. The container div is OUTSIDE the loop. --}}
                    @foreach ($experience->screenshots as $screenshot)
