...
@section('title', 'CMS Experience List')
...
<section class="bg-gradient-to-r from-purple-500 to-pink-500 py-20 md:py-32 flex items-center justify-center shadow-2xl mt-8">
    <h1 class="text-3xl sm:text-4xl md:text-5xl lg:text-7xl font-extrabold text-white text-center leading-tight drop-shadow-lg">
        LIST <span class="text-pink-800">YOUR</span> EXPERIENCES
    </h1>
</section>
    ...
    <div class="space-y-6">
        @forelse($experiences as $experience)
            <div class="flex flex-col sm:flex-row items-center justify-between p-6 bg-gray-800 rounded-xl shadow-2xl transition-transform duration-300 hover:scale-[1.02] cursor-pointer">
                ...
                <div class="flex flex-col sm:flex-row items-center gap-6 text-center sm:text-left mb-4 sm:mb-0">
                    <img src="{{ Storage::url($experience->cover) }}" alt="Project Cover" class="w-24 h-24 object-cover rounded-xl border-4 border-pink-500 shadow-lg"/>
                    <div class="flex flex-col justify-center">
                        <p class="font-bold text-white text-xl">{{ $experience->name }}</p>
                        <p class="text-gray-400">{{ $experience->tagline }}</p>
                        <p class="text-gray-400 text-sm">{{ $experience->datestart }} - {{ $experience->dateend }}</p>
                    </div>
                </div>
        ...
        @empty
            <div class="flex flex-col items-center justify-center py-16 bg-gray-800 rounded-xl shadow-2xl">
                <p class="text-4xl font-extrabold text-red-500 text-center mb-2">NO EXPERIENCE ADDED</p>
                <p class="text-xl text-gray-400 text-center">Get started by creating your work experiences.</p>
            </div>
        @endforelse
    </div>
</section>
